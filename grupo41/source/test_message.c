...
int testResult() {
	...
	int res_opcode = memcmp(msg_str, &opcode, 2);
	int res_ctype =  memcmp(msg_str + 2, &c_type, 2);
	int res_table_num = memcmp(msg_str + 4, &table_num, 2);
	int res_res = memcmp(msg_str + 6, &res, 4);
	result = res_opcode == 0 &&
		res_ctype == 0 && 
		res_table_num == 0 &&
		res_res == 0;
...
int testValue() {
	...
	int res_ctype =  memcmp(msg_str + 2, &c_type, 2);
	int res_table_num = memcmp(msg_str + 4, &table_num, 2);
	int res_datasize = memcmp(msg_str + 6, &datasize_conv, 4);
	int res_data = memcmp(msg_str + 10, &comp_data, datasize);
	result = res_opcode == 0 &&
		 res_ctype == 0 &&
		 res_table_num == 0 &&
		 res_datasize == 0 &&
...
int testEntry() {
	...
	int res_ctype = memcmp(msg_str + 2, &c_type, 2);
	int res_table_num =  memcmp(msg_str + 4, &table_num, 2);
	int res_keysize = memcmp(msg_str + 6, &keysize_conv, 2);
	int res_key =  memcmp(msg_str + 8, &comp_key, keysize);
	int res_datasize = memcmp(msg_str + 8 + keysize, &datasize_conv, 4);
	int res_data = memcmp(msg_str + 8 + keysize + 4, &comp_data, datasize);
